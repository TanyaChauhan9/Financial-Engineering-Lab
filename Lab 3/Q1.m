...
put=[];
for i=1:length(S0)
    [call(i),put(i)] = American_Option(S0(i),T,K_1,r_1,sigma_1,M_1);
end
fprintf('\n Varying S0 \n');
figure;
plot(S0,call);
...
put=[];
for i=1:length(K)
    [call(i),put(i)] = American_Option(S0_1,T,K(i),r_1,sigma_1,M_1);
end
figure;
plot(K,call);
xlabel('K')
...
put=[];
for i=1:length(r)
    [call(i),put(i)] = American_Option(S0_1,T,K_1,r(i),sigma_1,M_1);
end
figure;
plot(r,call);
xlabel('r')
...
for i=1:length(sigma)
    [call(i),put(i)] = American_Option(S0_1,T,K_1,r_1,sigma(i),M_1);
end
figure;
plot(sigma,call);
xlabel('sigma')
...
for k=95:5:105
    for i=1:length(M)
        [call(i),put(i)] = American_Option(S0_1,T,k,r_1,sigma_1,M(i));
    end
    figure;
    plot(M,call);
